...
echo "=== Set money=100 (1 silver) ==="
$D -e "UPDATE characters SET money=100 WHERE guid=1801" 2>/dev/null

echo "=== Verify ==="
$D -e "SELECT guid,name,money,at_login,playerFlags FROM characters WHERE guid=1801" 2>/dev/null

echo "=== Restart server ==="
docker restart cmangos-wotlk-server
sleep 30
docker ps --filter name=cmangos-wotlk-server --format "{{.Status}}"
echo "Login now. money=100 (1 silver)"
