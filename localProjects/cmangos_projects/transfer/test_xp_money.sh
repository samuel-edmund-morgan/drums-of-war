...
echo "=== Set xp + money ONLY ==="
$D -e "UPDATE characters SET xp=31, money=37903267 WHERE guid=1801" 2>/dev/null
echo "Set done"

echo "=== Verify ==="
$D -e "SELECT guid,name,at_login,playerFlags,xp,money,totaltime,leveltime FROM characters WHERE guid=1801" 2>/dev/null
